    ...
    <x-container class="max-w-screen-xl mx-auto">
        <section class="max-w-md sm:max-w-xl md:max-w-4xl xl:max-w-6xl mx-auto">
            ...
            <div class="flex gap-5">
                <div class="gap-5 md:columns-2 xl:columns-2 basis-3/4 [&>article:not(:first-child)]:mt-5">
                    @forelse ($posts as $post)
                        <article class="relative bg-white group h-fit overflow-hidden ">
                            <img src="{{ $post->image }}" alt="{{ $post->slug }}" class="object-cover w-full">
                            <a href="{{ route('posts.show', $post) }}"
                                class="absolute top-0 left-0 w-full h-full object-cover z-20"></a>
                            <div class="flex flex-col gap-2 py-3">
                                <h2 class="px-4 font-semibold capitalize text-lg border-l-2 border-yellow-400">
                                    {{ $post->tittle }}</h2>
                ...
                <div class="basis-1/3 bg-white p-3">
                        ...
                        @forelse ($posts_populer as $post_populer)
                            <article class="relative flex gap-2">
                                <img src="{{ $post_populer->image }}" alt="{{ $post_populer->slug }}"
                                    ...
                                    <a href="{{ route('posts.show', $post_populer) }}" class="absolute top-0 left-0 w-full h-full"></a>
