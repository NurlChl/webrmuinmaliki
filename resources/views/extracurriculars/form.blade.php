                    ...
                    <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                        <div class="sm:col-span-2">
                            <label for="name" :value="__('name')"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama UKM</label>
                            <input type="text" name="name" id="name"
                                ...
                                placeholder="Nama UKM" required>
                            <x-input-error :messages="$errors->get('name')" />
                        </div>
                        <div class="sm:col-span-2">
                            <label for="leader_name" :value="__('leader_name')"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Ketua</label>
                            <input type="text" name="leader_name" id="leader_name"
                                value="{{ old('leader_name', $extracurricular->leader_name) }}"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-emerald-600 focus:border-emerald-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-emerald-500 dark:focus:border-emerald-500"
                                placeholder="Nama Ketua" required>
                            <x-input-error :messages="$errors->get('leader_name')" />
                        </div>
                        <div class="sm:col-span-2">
                            <div class="col-span-full">
                                <label for="image"
                                    class="block text-sm font-medium leading-6 text-gray-900">Foto Ketua</label>
                                <div
                                    class="mt-2 flex justify-center rounded-lg border border-dashed border-gray-900/25 px-6 py-10">
                                    <div class="text-center">
                                        <svg id="svg-img" class="{{ isset($extracurricular->image) ? 'hidden' : ''}} mx-auto h-12 w-12 text-gray-300" viewBox="0 0 24 24"
                                            fill="currentColor" aria-hidden="true">
                                            <path fill-rule="evenodd"
                                                d="M1.5 6a2.25 2.25 0 012.25-2.25h16.5A2.25 2.25 0 0122.5 6v12a2.25 2.25 0 01-2.25 2.25H3.75A2.25 2.25 0 011.5 18V6zM3 16.06V18c0 .414.336.75.75.75h16.5A.75.75 0 0021 18v-1.94l-2.69-2.689a1.5 1.5 0 00-2.12 0l-.88.879.97.97a.75.75 0 11-1.06 1.06l-5.16-5.159a1.5 1.5 0 00-2.12 0L3 16.061zm10.125-7.81a1.125 1.125 0 112.25 0 1.125 1.125 0 01-2.25 0z"
                                                clip-rule="evenodd" />
                                        </svg>
                                        <img class="img-preview max-w-full" @if (isset($extracurricular->image)) src="{{ Storage::url($extracurricular->image) }}" @endif />
                                        <div class="mt-4 flex text-sm leading-6 text-gray-600">
                                            <label for="image" :value="__('image')"
                                                ...
                                                <span id="upload-label">
                                                    {{ isset($extracurricular->image) ? 'Ganti gambar' : 'Upload gambar' }}
                                                </span>
                        ...
                        <div class="sm:col-span-2">
                            <label for="contact" :value="__('contact')"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">No. Telp (CP)</label>
                            <input type="number" name="contact" id="contact" value="{{ old('contact', $extracurricular->contact) }}"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-emerald-600 focus:border-emerald-600 block w-full p-2.5"
                                placeholder="Contoh: 6289234868923" required>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="body" id="body"
                                class="text-sm rounded-lg block w-full p-2.5 bg-gray-50 border border-gray-300 text-gray-900 focus:ring-emerald-500 focus:border-emerald-500"
                                placeholder="Deskripsi UKM" required>{{ old('body', $extracurricular->body) }}</textarea>
