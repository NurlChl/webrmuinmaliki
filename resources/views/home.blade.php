<x-app-layout>
    ...
    <section class="relative py-20 sm:py-40 lg:py-52 bg-no-repeat bg-cover bg-center"
        style="background-image: url('http://projekuin.test/logo/uin.jpeg')">
    ...
    <x-container class="flex flex-col lg:flex-row gap-10 mx-auto max-w-3xl lg:w-full lg:max-w-7xl mt-7 sm:mt-10">

        <section class="max-w-screen-md basis-3/4 flex flex-col gap-7 sm:gap-10 w-full">

            @if ($posts->count() > 2)
                <div>
                    <h2 class="mb-4 text-base text-red-600 font-bold uppercase">Highlight Kegiatan</h2>
                    <div id="default-carousel" class="relative w-full" data-carousel="slide">
                        ...
                        <div
                            ...
                            @foreach ($carousels as $carousel)
                                <button type="button" class="w-2 h-2 sm:w-3 sm:h-3 rounded-full" aria-current="true"
                        ...
                        <article class="flex flex-col group">
                            <a href="{{ route('posts.show', $post) }}">
                                <h2
                                    ...
                                    {{ str()->limit($post->tittle, 160) }}</h2>
                            </a>
                            <div class="pl-4">
                                <span
                                    class="text-sm font-semibold text-emerald-900">{{ $post->created_at->format('d-m-Y') }}</span>
                            </div>
                            <p class="text-base pl-4 text-zinc-600">{{ $post->excerpt }}</p>
                        </article>
                    @endforeach
                </div>
            </div>
        ...
        <section class="flex flex-col gap-10 basis-1/3 w-full">
            ...
            <div class="p-5 bg-white rounded-lg shadow-lg border border-emerald-300">
                    ...
                    @forelse ($rulesByCategory as $ruleByCategory => $rules )
                        @forelse ($rules as $rule)
                            <div class="flex flex-row gap-5 relative bg-slate-100 py-3 px-5 rounded-md group">
                                <a href="{{ $rule->file }}" target="_blank" class="absolute top-0 left-0 w-full h-full"></a>
                                <svg class="w-6 h-6 text-amber-400 group-hover:text-amber-500 transition-all ease-in-out duration-300" aria-hidden="true"
                                    xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                    fill="currentColor" viewBox="0 0 24 24">
                                    <path
                                ...
                                <p class="text-sm font-semibold group-hover:text-emerald-600 transition ease-in-out duration-300">{{ $rule->name }}</p>
                            </div>
                ...
                <div class="mt-5 text-end">
                    <a href="{{ route('rules.index') }}" class=" text-xs text-emerald-600 font-semibold ">Muat lebih banyak ></a>
                </div>
            </div>
